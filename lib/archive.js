...
import { readPointerFile, writePointerFile, checkStepsCompletion } from './task.js'
...
export async function archiveTaskRecord(dshFs, root, args, exec = {}, sandboxPolicy, opts = {}) {
  ...
  try {
    ...
    if (info) {
      ...
      taskSteps = parsed && Array.isArray(parsed.steps) ? parsed.steps : null
    }
  } catch {
  ...
  // Steps completion gate: a task with steps must have them all completed and
  // verified before it can be archived (mirrors the updateTaskRecord guard).
  const stepsGate = checkStepsCompletion(taskSteps)
  if (!stepsGate.ok) {
    return { ok: false, error: stepsGate.error }
  }
