...
export const STEP_STATUSES = ['pending', 'in_progress', 'verifying', 'blocked', 'completed']

/** Verification modes for a task step (ai = automated/model-run, human = manual approval gate). */
export const STEP_VERIFICATION_TYPES = ['none', 'ai', 'human']
...
export function validateSteps(steps) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    if (typeof item.verification === 'string') {
      const verification = item.verification.trim()
      if (!STEP_VERIFICATION_TYPES.includes(verification)) {
        return { ok: false, error: `steps[${i}].verification 必须是 ${STEP_VERIFICATION_TYPES.join(' | ')} 之一（当前：${verification}）` }
      }
    ...
    if (item.verifiedBy === 'ai' || item.verifiedBy === 'human') {
      step.verifiedBy = item.verifiedBy
    }
    ...
    if (item.blockedReason && typeof item.blockedReason === 'string' && item.blockedReason.trim()) {
      step.blockedReason = item.blockedReason.trim()
    }
...
export function validateStepUpdate(update) {
  ...
  if (update.verification !== undefined) {
    ...
    if (!STEP_VERIFICATION_TYPES.includes(verification)) {
      return { ok: false, error: `step.verification 必须是 ${STEP_VERIFICATION_TYPES.join(' | ')} 之一（当前：${verification}）` }
    }
  ...
  if (update.verifiedBy === 'ai' || update.verifiedBy === 'human') {
    out.verifiedBy = update.verifiedBy
  }
  ...
  if (update.blockedReason !== undefined && typeof update.blockedReason === 'string') {
    out.blockedReason = update.blockedReason.trim()
  }
  if (
    out.status === undefined &&
    out.verification === undefined &&
    out.verified === undefined &&
    out.verifiedBy === undefined &&
    out.verificationNotes === undefined &&
    out.blockedReason === undefined
  ) {
    return { ok: false, error: 'step 更新必须至少包含 status、verification、verified、verifiedBy、verificationNotes 或 blockedReason 之一' }
  }
 ...
 *
 * Two-phase commit is enforced: a step with a verification requirement can only
 * be marked `completed` when `verified === true` is ALREADY present in the
 * persisted state (recorded by a prior update call), preventing self-
 * certification in a single call.
 *
 * Verification subject (`verification`):
 *  - 'ai'    — model-run automated verification (verify: true is a legacy alias
 *              for this); marking `verified: true` is the model's own record.
 *  - 'human' — manual approval gate: `verified: true` REQUIRES `verifiedBy:
 *              'human'` (the human explicitly confirmed in the conversation),
 *              otherwise the model cannot self-certify the step.
 *  - 'none'  — no verification requirement; may go straight to completed.
 *
 * Blocked linkage: a step moved to `blocked` must carry a `blockedReason`
 * (either in this update or already persisted).
...
export function applyStepUpdate(existingSteps, update) {
  ...
  // Verification mode resolution: explicit `verification` wins over the legacy
  // `verify: true` alias (verify: true === verification: 'ai').
  const verification =
  ...
  if (update.verification !== undefined) {
    next.verification = update.verification
  }
  if (update.verified !== undefined) {
    if (update.verified === true && verification === 'human' && update.verifiedBy !== 'human') {
      return {
        ok: false,
        error: `[trellis/human_gate] 步骤 "${current.title}" (${current.id}) 声明了人工验收要求 (verification: human)，标记 verified: true 必须同时提供 verifiedBy: 'human'（用户在对话中的明确确认），禁止模型自证自签`,
      }
    }
    next.verified = update.verified === true
    if (update.verified === true) {
      next.verifiedBy = update.verifiedBy === 'human' ? 'human' : 'ai'
    }
  }
  if (update.verifiedBy !== undefined) {
    next.verifiedBy = update.verifiedBy
  }
  if (update.verificationNotes !== undefined) {
    next.verificationNotes = update.verificationNotes
  }
  if (update.blockedReason !== undefined) {
    next.blockedReason = update.blockedReason
  }

  if (update.status !== undefined) {
    if (update.status === 'completed') {
      if (verification === 'ai' && current.verified !== true) {
        return {
          ok: false,
          error: `[trellis/verification_gate] 步骤 "${current.title}" (${current.id}) 声明了自动化验证要求 (verification: ai)，请先单独调用 trellis_task_update 记录 verified: true 后再标记 completed`,
        }
      }
      if (verification === 'human' && (current.verified !== true || current.verifiedBy !== 'human')) {
        return {
          ok: false,
          error: `[trellis/human_gate] 步骤 "${current.title}" (${current.id}) 声明了人工验收要求 (verification: human)，在用户明确确认并持久化 verified: true + verifiedBy: 'human' 前禁止标记为 completed`,
        }
      }
    }
    if (update.status === 'blocked' && !next.blockedReason) {
      return {
        ok: false,
        error: `[trellis/steps_blocked] 步骤 "${current.title}" (${current.id}) 标记为 blocked 时必须提供 blockedReason 说明阻塞原因`,
      }
    }
...
export function checkStepsCompletion(steps) {
  ...
  for (const st of steps) {
    if (st.status === 'blocked') {
      blocked.push(`${st.title || st.id}${st.blockedReason ? `（${st.blockedReason}）` : ''}`)
      continue
    }
    if (st.status !== 'completed') {
      incomplete.push(`${st.title || st.id} (${st.status || 'pending'})`)
    } else if (st.verify === true || st.verification === 'ai' || st.verification === 'human') {
      if (st.verified !== true) {
        unverified.push(`${st.title || st.id}`)
      } else if (st.verification === 'human' && st.verifiedBy !== 'human') {
        unverified.push(`${st.title || st.id}（人工验收未确认）`)
      }
    }
  }
  if (blocked.length > 0) {
    return { ok: false, error: `[trellis/steps_blocked] 以下步骤处于阻塞状态，禁止提前完结任务：${blocked.join('；')}` }
  }
...
export function validateCreateArgs(args) {
  ...
  // status↔stage consistency (write-path hardening): a planning-typed stage
  // (feat prd/design/design-review; issue report/analyze; refactor scan/design)
  // is a read-only window and may never be created as in_progress — otherwise
  // the read-only guard would treat it as executing. See lib/state.js stagePhase.
  const effectiveStage = stage || fallbackStage(workType, status)
  if (status === 'in_progress' && stagePhase(workType, effectiveStage) === 'planning') {
    const track = TRACKS[workType]
    return {
      ok: false,
      error:
        `status=in_progress 与规划型阶段 "${effectiveStage}" 冲突：${workType} 的 ` +
        `${effectiveStage} 属规划期（只读，禁止写码）。请保持 status=planning 推进到设计批准，` +
        `或直接以写码阶段（如 ${track ? track.in_progress : '?'}）创建。`,
    }
  }
 ...
 * `current_task` and `skipped` are mutually exclusive: writing a non-null
 * task clears any skip flag; writing skip (via skipTaskPointer) clears the
 * task pointer.
 * @param {import('@deepseek-ai/dsh-fs').FileSystem} dshFs ctx.fs.
 * @param {string} absPath pointer file path.
 * @param {string | null} currentTask the task dir reference (".trellis/tasks/<slug>") or null.
 * @param {object} [opts] { signal?, sandboxPolicy?, skipped? } — skipped=true also clears current_task.
 * @returns {Promise<void>}
 */
export async function writePointerFile(dshFs, absPath, currentTask, { signal, sandboxPolicy, skipped } = {}) {
  const existing = await readPointerFile(dshFs, absPath)
  let next = existing ? { ...existing } : {}
  if (skipped === true) {
    next.current_task = null
    next.skipped = true
  } else {
    next.current_task = currentTask
    if (currentTask) next.skipped = false
  }
  const target = await dshFs.resolve(absPath)
...
/**
 * Mark a session as "skip the task workflow" (authorized to write without a
 * task). Writes `skipped: true` and clears `current_task` on the session's own
 * pointer file. Refuses when the session is bound to an active task.
 * @param {import('@deepseek-ai/dsh-fs').FileSystem} dshFs ctx.fs.
 * @param {string} root normalized project root.
 * @param {string | undefined} sessionId live session id.
 * @param {object} [opts] { signal?, sandboxPolicy? }.
 * @returns {Promise<{ ok: true, sessionFile: string, skipped: true } | { ok: false, error: string }>}
 */
export async function skipTaskPointer(dshFs, root, sessionId, opts = {}) {
  const runtimeDir = path.join(root, '.trellis', '.runtime', 'sessions')
  const name = `${sessionFileBasename(sessionId)}.json`
  const absPath = path.join(runtimeDir, name)
  const existing = await readPointerFile(dshFs, absPath)
  if (existing && typeof existing.current_task === 'string' && existing.current_task.length > 0) {
    return { ok: false, error: '当前会话已绑定活动任务，不能跳过任务；请先解除绑定或继续该任务' }
  }
  await writePointerFile(dshFs, absPath, null, { ...opts, skipped: true })
  return { ok: true, sessionFile: name, skipped: true }
}

/**
 * Clear a session's skip flag (returning it to the undecided/read-only state
 * without binding a task). Used by the unbind path when a skipped session is
 * re-bound to a task — writePointerFile already clears skipped when a task is
 * written, so this is a no-op safety export for the read path.
 * @param {import('@deepseek-ai/dsh-fs').FileSystem} dshFs ctx.fs.
 * @param {string} root normalized project root.
 * @param {string | undefined} sessionId live session id.
 * @param {object} [opts] { signal?, sandboxPolicy? }.
 * @returns {Promise<{ ok: true, sessionFile: string, skipped: false }>}
 */
export async function clearSkipPointer(dshFs, root, sessionId, opts = {}) {
  const runtimeDir = path.join(root, '.trellis', '.runtime', 'sessions')
  const name = `${sessionFileBasename(sessionId)}.json`
  const absPath = path.join(runtimeDir, name)
  const existing = await readPointerFile(dshFs, absPath)
  const next = existing ? { ...existing, current_task: null, skipped: false } : { current_task: null, skipped: false }
  const target = await dshFs.resolve(absPath)
  await dshFs.writeText(target, JSON.stringify(next, null, 2), undefined, opts.signal, opts.sandboxPolicy)
  return { ok: true, sessionFile: name, skipped: false }
}
...
export async function updateTaskRecord(dshFs, root, args, exec = {}, sandboxPolicy) {
  ...
  // status↔stage consistency (write-path hardening), validated against the
  // MERGED state so an update flipping only status or only stage is still
  // caught: a planning-typed stage (read-only window) may never be stored
  // with status=in_progress. See lib/state.js stagePhase.
  const mergedStatus = args.status !== undefined ? args.status : taskJson.status
  const mergedStage =
    args.stage !== undefined
      ? args.stage
      : taskJson.work && typeof taskJson.work.stage === 'string'
        ? taskJson.work.stage
        : fallbackStage(workType, mergedStatus)
  if (mergedStatus === 'in_progress' && stagePhase(workType, mergedStage) === 'planning') {
    const track = TRACKS[workType]
    return {
      ok: false,
      error:
        `status=in_progress 与规划型阶段 "${mergedStage}" 冲突：${workType} 的 ` +
        `${mergedStage} 属规划期（只读）。请先推进到写码阶段（如 ${track ? track.in_progress : '?'}）再写 in_progress。`,
    }
  }
