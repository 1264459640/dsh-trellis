...
export function validateUpdateArgs(args) {
  ...
  return {
    ok: true,
    args: { ...args, slug, status, mode, title, stage, description, force, modified_files: modifiedFiles },
  }
}
...
export async function updateTaskRecord(dshFs, root, args, exec = {}, sandboxPolicy) {
  ...
  // If updating to status='completed', enforce git cleanliness check unless force=true
  if (args.status === 'completed' && !args.force) {
