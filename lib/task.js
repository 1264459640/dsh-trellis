...
import { todayMmDd, validateSlug, fallbackStage, stageOnTrack, TRACKS } from './state.js'
 ...
 */
async function readPointerFile(dshFs, absPath) {
  try {
 ...
 */
async function writePointerFile(dshFs, absPath, currentTask, { signal, sandboxPolicy } = {}) {
...
export async function createTaskRecord(dshFs, root, args, exec = {}, sandboxPolicy) {
  ...
  // 4. Runtime session pointer(s): canonical dsh-session.json (deterministic
  //    picker wins when present) + per-session file when a session id is known.
