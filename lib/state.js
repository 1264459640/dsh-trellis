...
/**
 * Parse a resolved runtime session file into its active-task pointer and
 * skip state. `current_task` and `skipped` are mutually exclusive (enforced
 * by the writer in lib/task.js): a non-empty current_task means the session
 * is on the task track; skipped:true means the user authorized skipping it.
 * @param {any} json the parsed session JSON.
 * @returns {{ taskDir?: string | null, skipped?: boolean }} pointer + skip flag.
 */
export function activeTaskFromSession(json) {
  if (!json || typeof json !== 'object') return {}
  const task = json.current_task
  if (typeof task === 'string' && task.length > 0) {
    return { taskDir: task, skipped: json.skipped === true }
  }
  return { skipped: json.skipped === true }
}
...
/**
 * Classify a work-type stage as planning (read-only) or in_progress
 * (writable), following the status↔stage mapping in
 * `skills/_templates/work-types.md`:
 ...
 * The boundary is the track's first writable stage (`TRACKS[type].in_progress`):
 * stages strictly before it are planning, from it onward in_progress. The
 * display terminals 'finish'/'done' live outside the stages array — for a
 * completed task `phaseForTask` lets status win, so they only fall through in
 * degenerate non-completed states (falling back to the status-based phase).
 * @param {string | null | undefined} workType feat | issue | refactor.
 * @param {string | null | undefined} stage task.json work.stage.
 * @returns {'planning' | 'in_progress' | null} null when the type/stage is unknown.
 */
export function stagePhase(workType, stage) {
  const track = TRACKS[workType]
  if (!track || typeof stage !== 'string') return null
  const codeIdx = track.stages.indexOf(track.in_progress)
  const idx = track.stages.indexOf(stage)
  if (idx === -1) return null
  return idx < codeIdx ? 'planning' : 'in_progress'
}

/**
 * Resolve the workflow phase from the task's fine-grained position: `work.stage`
 * classifies planning vs in_progress via the work-type track (see stagePhase),
 * so a refactor task at scan stays read-only even if `status` drifted to
 * in_progress. `status === 'completed'` always wins (display terminal). Falls
 * back to the coarse status-based `phaseFor` when the stage is unknown or
 * off-track (legacy tasks / missing work block).
 * @param {any} task parsed task.json (object or null).
 * @param {boolean} [inline] codex-inline dispatch flag.
 * @returns {string} one of PHASES.
 */
export function phaseForTask(task, inline = false) {
  if (!task || typeof task !== 'object') return 'no_task'
  ...
  const sp = stagePhase(workType, stage)
  if (sp === 'planning') return inline ? 'planning-inline' : 'planning'
  if (sp === 'in_progress') return inline ? 'in_progress-inline' : 'in_progress'
  return phaseFor(status, inline)
}
