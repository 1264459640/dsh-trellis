 ...
 *  3. registers a `trellis_state` diagnostic tool and a `trellis_task_create`
 *     tool (task.json + runtime session pointer in one call);
...
export default {
  ...
  apply(ctx, config) {
    ...
    const archiveTool = defineTool({
      ...
      async execute(args, exec) {
        ...
        try {
          ...
          // (the task-state route refetches on cache miss).
    ...
    ctx.inject(['webServer', 'sessions'], (web) => {
       ...
       * `.trellis/tasks/archive/<yyyy-mm>/*` (slug, title, status, work type,
       * stage, present artifacts, month key, archived flag) plus the current
       * session's active task. Paths never reach the browser.
       *
       * The month key follows the SAME logic as the archive operation
       * (lib/archive.js): an archived task's key is its parent bucket folder
       * name (`yyyy-mm`, authoritative); a task still in the active tree gets
       * `ymKeyFromSlug` (slug month + current year) so a completed task shows
       * in the same bucket the archive tool would write it to.
       * @param {import('@deepseek-ai/dsh-fs').FileSystem} fs
       * @param {string} root normalized project root.
       * @param {string | undefined} sessionId live session id.
       * @param {boolean} [inline] codex-inline dispatch flag.
       * @returns {Promise<object>} board payload (kind: 'board') or a stable
       *   empty kind when nothing is readable.
       */
      const buildBoard = async (fs, root, sessionId, inline = false) => {
        const p = trellisPaths(root)
        /**
         ...
         */
        const readTask = async (dirEntry, meta) => {
          // listDir entries carry `target: { targetKey, displayPath }` —
          // join on the local path key, never on the object itself.
          const taskJsonTarget = await fs.resolve(path.join(dirEntry.target.targetKey, 'task.json'))
          const info = await fs.stat(taskJsonTarget)
          if (!info) return null
          const parsed = JSON.parse((await fs.readText(taskJsonTarget)).replace(/^\uFEFF/, ''))
        ...
        try {
          ...
          for (const entry of entries) {
            ...
            // archive/ is the read tree, never a task itself.
            if (entry.name === 'archive' || entry.name.startsWith('.')) continue
            try {
              addTask(await readTask(entry, { month: ymKeyFromSlug(entry.name), archived: false }))
            } catch {
              /* skip malformed task dir */
            }
          }
        } catch {
          /* no tasks dir */
        ...
        try {
          ...
          for (const bucket of buckets) {
            ...
            let taskEntries = []
            try {
              taskEntries = await fs.listDir(bucket.target)
            } catch {
              continue
            }
            for (const taskEntry of taskEntries) {
              ...
              try {
                addTask(await readTask(taskEntry, { month: bucket.name, archived: true }))
              } catch {
                /* skip malformed archived task dir */
              }
            }
          }
        } catch {
        ...
        // Flush once, after BOTH trees: the archive copy wins for a task that
        // appears in both (mid-migration), and a missing archive dir never
        // hides the live board.
        ...
        try {
          ...
          for (const entry of entries) {
            ...
            try {
              ...
              sessions.push({ name: entry.name, taskDir: taskDir || null })
            } catch {
              sessions.push({ name: entry.name, taskDir: null })
            }
          }
          ...
          if (picked) currentTask = String(picked).replace(/\\/g, '/').split('/').filter(Boolean).pop() || null
        } catch {
          /* no runtime dir */
        }
