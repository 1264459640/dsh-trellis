 ...
 *     (session file → active task → task.json → stage-aware phase) and workflow.md breadcrumb,
...
import { shouldSkip, buildBreadcrumbMessage, findActiveStep, formatStepPrompt, formatStepReminder } from './breadcrumb.js'
...
import { createArtifactUpdateTool } from './artifact.js'
import {
  applyReadonlyPolicy,
...
async function resolveProjectState(fs, root, inline = false, sessionId = undefined) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const { taskDir, skipped } = activeTaskFromSession(JSON.parse(text))
        parsed.push({ name: entry.name, taskDir: taskDir || null, skipped: skipped === true })
        if (entry.name === `${sessionFileBasename(sessionId)}.json`) {
          skipState = skipped === true
        }
      } catch {
        parsed.push({ name: entry.name, taskDir: null, skipped: false })
      }
    }
  ...
  //    Phase is stage-aware (phaseForTask): a refactor task at scan/design or
  //    an issue at report/analyze stays in the planning (read-only) phase even
  //    if task.status drifted to in_progress, per work-types.md's mapping.
  const phase = phaseForTask(task, inline)
  ...
  return { phase, activeTask, slug, slugCheck, text, source: crumb.source, task, skipState }
}
...
export default {
  ...
  apply(ctx, config) {
    ...
    const createTool = defineTool({
      ...
      parameters: {
        ...
        steps: {
          ...
          description: 'Optional initial execution steps: each step needs id, title and at least one measurable acceptance criterion.',
        },
        cwd: { type: 'string', description: 'Optional absolute project dir. Defaults to the calling session cwd.' },
      },
      output: {
      ...
      async execute(args, exec) {
        ...
        try {
          ...
          if (sessionId) {
            const phase = phaseForTask(result.taskJson, effectiveConfig.get().inline === true)
          ...
          return {
            ...
            phase: phaseForTask(result.taskJson, effectiveConfig.get().inline === true),
    ...
    // --- task skip tool ------------------------------------------------------
    // `trellis_task_skip`: authorize THIS session to write without going
    // through the task workflow (the `no_task` read-only fallback's escape
    // hatch). Writes `skipped: true` on the session's own pointer file and
    // clears `current_task`, moving the session from undecided → authorized.
    // One-way: after skipping, the only way back onto the task track is
    // `trellis_task_create`. Ask for user consent BEFORE calling.
    const skipTool = defineTool({
      name: 'trellis_task_skip',
      description:
        'Authorize this session to work without a Trellis task (skip the task workflow for this conversation): writes `skipped: true` to this session\'s runtime pointer and clears any task binding, so the read-only undecided tool surface unlocks to full write access. Only valid while the session has NO active task; if the session is bound to a task, this is rejected. One-way: after skipping, create a task via `trellis_task_create` to re-enter the task track. ASK FOR USER CONSENT before calling — skipping waives planning-phase write protection for this session.',
      parameters: {
        cwd: { type: 'string', description: 'Optional absolute project dir. Defaults to the calling session cwd.' },
      },
      output: {
        schema: {
          ...
          properties: {
            ...
            error: { oneOf: [{ type: 'string' }, { type: 'null' }] },
            project: { oneOf: [{ type: 'string' }, { type: 'null' }] },
            sessionFile: { oneOf: [{ type: 'string' }, { type: 'null' }] },
      ...
      async execute(args, exec) {
        ...
        const root = matchAllowlist(cwd, effectiveConfig.get().allowlist)
        if (!root) {
          return { ok: false, error: '项目不在 allowlist 内；请先在 Web 设置或配置里把项目根加入 trellis-workflow.allowlist', project: null, sessionFile: null, skipped: false }
        }
        const sessionId =
          exec.agent && exec.agent.session && typeof exec.agent.session.id === 'string'
            ? exec.agent.session.id
        ...
        const policy = ctx.fs.sandboxMode === void 0 ? void 0 : ctx.get('sandboxPolicy')
        if (ctx.fs.sandboxMode !== void 0 && policy === void 0) {
          throw new Error('trellis_task_skip: the mounted filesystem confines but ctx.sandboxPolicy is missing')
        }
        const sandboxPolicy = policy
          ? policy.resolve({ ...(exec.agent === void 0 ? {} : { session: exec.agent.session }) })
          : void 0
        try {
          const result = await skipTaskPointer(ctx.fs, root, sessionId, {
            signal: exec.signal,
            sandboxPolicy,
          })
          if (!result.ok) {
            return { ok: false, error: result.error, project: root, sessionFile: null, skipped: false }
          }
          // Refresh the Web chip cache: a skipped session is still "no task",
          // but if it was previously bound, the cache must reset. The chip
          // reads taskSummaryOf with the live phase; skipped only affects the
          // tool surface (authorizationOf), not the displayed phase.
          if (sessionId) {
            lruSet(summaries, sessionId, taskSummaryOf(null, { matched: true, phase: 'no_task' }), CACHE_LIMIT)
          }
          return { ok: true, error: null, project: root, sessionFile: result.sessionFile, skipped: true }
        } catch (error) {
          if (error && error.code === 'FS_SANDBOX_DENIED') {
            const mode = sandboxPolicy && sandboxPolicy.mode !== void 0 ? sandboxPolicy.mode : 'unknown'
            throw new Error(`[sandbox: file access denied under ${mode} mode]`)
    ...
    const disposeSkipTool = ctx.tools.register(skipTool)
    ctx.effect(() => disposeSkipTool)
    ...
    const updateTool = defineTool({
      ...
      parameters: {
        ...
        steps: {
          ...
          description: 'Optional full replacement of the execution steps list.',
        },
        step: {
          type: 'object',
          additionalProperties: false,
          properties: {
            id: { type: 'string', description: 'Step id to update (required).' },
            status: { type: 'string', description: 'pending | in_progress | verifying | blocked | completed.' },
            verification: { type: 'string', description: 'none | ai | human.' },
            verified: { type: 'boolean', description: 'Mark the step verification as passed (true) when verify/verification is required.' },
            verifiedBy: { type: 'string', description: 'ai | human.' },
            verificationNotes: { type: 'string', description: 'Verification evidence (test cases / logs) or human review notes.' },
            blockedReason: { type: 'string', description: 'Reason if step is marked as blocked.' },
          },
          description: 'Optional incremental update to a single step. A step with verification/verify cannot reach completed until verified: true is recorded via a prior call.',
        },
    ...
    // In-memory per-session snapshot of the last announced active step
    // (key: `${stepId}:${status}:${verified}`) — powers the breadcrumb
    // step-prompt dedup so an unchanged step degrades to a one-line reminder
    // instead of re-flooding the context. Pure memory, zero disk IO.
    const stepInjectedCache = new Map()
    ...
    // --- trellis_artifact_update: sandboxed artifact write channel ----------
    const artifactTool = createArtifactUpdateTool({ ctx, effectiveConfig })
    const disposeArtifactTool = ctx.tools.register(artifactTool)
    ctx.effect(() => disposeArtifactTool)
    ...
    ctx.inject(['webServer', 'sessions'], (web) => {
      ...
      web.on('session/disposed', (session) => {
        if (session && typeof session.id === 'string') {
          summaries.delete(session.id)
          stepInjectedCache.delete(session.id)
        }
      })
    ...
    // --- Read-only planning enforcement -------------------------------------
    // When enforceReadonlyPlanning is enabled and the session cwd matches the
    // allowlist, TRIM the tool surface by AUTHORIZATION state (lib/readonly.js)
    // using DENYLIST semantics — only the specified tools are removed:
    //   undecided (no task, not skipped) → trim write/edit + task-write trellis tools
    //   planning (task in planning)       → trim write/edit + create/skip/archive trellis tools
    //   authorized (approved / skipped)   → full tool surface, no trimming
    // Every other tool — including tools registered by other plugins
    // (web_search, generate_image, subagent, skill, ...) — is preserved.
    // The display phase (lib/state.js) is untouched; only the tool surface is
    // pruned. Additionally, for undecided/planning states an explicit
    // read-only instruction section is APPENDED (appendReadonlyInstruction),
    // so the model sees its read-only state in the prompt text, not just a
    // pruned tool surface; authorized stays byte-identical to the unpruned
    // assembly. Gated by the enforceReadonlyPlanning master switch.
    ctx.on('system-prompt/assemble', async (assembly, context, next) => {
      const assembled = await next()
      const effective = effectiveConfig.get()
      if (!effective.enforceReadonlyPlanning) return assembled

      const agent = context && context.agent
      const cwd = normalizePath(agent && agent.session && agent.session.header ? agent.session.header.cwd : undefined)
      const root = matchAllowlist(cwd, effective.allowlist)
      if (!root) return assembled

      const sessionId = agent && agent.session && typeof agent.session.id === 'string' ? agent.session.id : undefined
      const st = await resolveProjectState(ctx.fs, root, effective.inline === true, sessionId)
    ...
    ctx.on('agent/pre-step', async ({ agent, messages, step, signal }, next) => {
      ...
      // --- active step injection (in_progress tasks with steps) --------------
      // Inject the single active step's acceptance criteria into the breadcrumb.
      // Dedup via an in-memory per-session snapshot key: when the same step in
      // the same state was already announced, degrade to a one-line reminder
      // instead of re-flooding the context window. No disk IO here.
      let stepInfo = null
      let stepPromptOverride = undefined

      if (
        st.task &&
        Array.isArray(st.task.steps) &&
        st.task.steps.length > 0 &&
        (st.phase === 'in_progress' || st.phase === 'in_progress-inline')
      ) {
        stepInfo = findActiveStep(st.task.steps)
        if (stepInfo) {
          const sid = agent && agent.session && typeof agent.session.id === 'string' ? agent.session.id : '_anon'
          const snapshotKey = `${stepInfo.step.id}:${stepInfo.step.status}:${stepInfo.step.verified ? 'v' : 'u'}`
          if (stepInjectedCache.get(sid) === snapshotKey) {
            // Same step, same state — already announced; keep it to one line.
            stepPromptOverride = formatStepReminder(stepInfo.step)
          } else {
            stepInjectedCache.set(sid, snapshotKey)
            stepPromptOverride = formatStepPrompt(stepInfo)
          }
        }
      }
