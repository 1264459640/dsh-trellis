...
import { validateCreateArgs, createTaskRecord } from './task.js'
 ...
 */
async function resolveProjectState(fs, root, inline = false) {
  ...
  try {
    ...
    const picked = activeTaskPointer(parsed)
    if (picked) activeTask = picked.taskDir
  } catch {
    /* no runtime dir */
  }
  let status = null
  ...
  apply(ctx, config) {
    ...
    const stateTool = defineTool({
      ...
      async execute(args, exec) {
        ...
        const st = await resolveProjectState(ctx.fs, root, args.inline === true)
        return {
          project: root,
          phase: st.phase,
    ...
    ctx.inject(['webServer', 'sessions'], (web) => {
      ...
      const refreshSummary = async (sessionId, inline = false) => {
        ...
        const st = await resolveProjectState(web.fs, root, inline)
    ...
    ctx.on('agent/pre-step', async ({ agent, messages, step, signal }, next) => {
      ...
      const st = await resolveProjectState(ctx.fs, root, inline)
