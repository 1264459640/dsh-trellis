  ...
  apply(ctx, config) {
    ...
    const createTool = defineTool({
      ...
      parameters: {
        ...
        steps: { type: 'array', description: 'Optional initial execution steps: [{ id, title, acceptance: string[], status?, verify?, verified?, verificationNotes? }]. Each step needs at least one measurable acceptance criterion.' },
        cwd: { type: 'string', description: 'Optional absolute project dir. Defaults to the calling session cwd.' },
      },
      output: {
        schema: {
    ...
    const updateTool = defineTool({
      ...
      parameters: {
        ...
        steps: { type: 'array', description: 'Optional full replacement of the execution steps list: [{ id, title, acceptance: string[], status?, verify?, verified?, verificationNotes? }].' },
        step: { type: 'object', description: 'Optional incremental update to a single step: { id, status?, verified?, verificationNotes? }. Steps with verify: true cannot be marked completed until verified: true is recorded.' },
