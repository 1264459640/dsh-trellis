...
export default {
  ...
  apply(ctx, config) {
    ...
    ctx.inject(['webServer', 'sessions'], (web) => {
       ...
       * under `.trellis/tasks/` (slug, title, status, work type, stage,
       * present artifacts, month key for the archive tree) plus the current
        ...
        try {
          ...
          for (const entry of entries) {
            ...
            try {
              ...
              const taskJsonTarget = await fs.resolve(path.join(entry.target.targetKey, 'task.json'))
              ...
              let artifacts = []
              try {
                const files = await fs.listDir(entry.target)
              ...
              tasks.push({
                slug: entry.name,
                title:
                  parsed.title && typeof parsed.title === 'string' && parsed.title ? parsed.title : entry.name,
                status,
                workType: workType || null,
                stage: stage || null,
                month: monthKeyFromSlug(entry.name),
                artifacts,
              })
            } catch {
        ...
        handler: async (req, res) => {
          try {
            ...
            if (pathname === TASK_STATE_PATH) {
              ...
              const cached = sessionId ? summaries.get(sessionId) : undefined
              // Liveness + cache: either missing -> the stable no-summary empty
              // state (indistinguishable from an unknown session, no probing).
              const value =
                cached && sessionId && web.sessions.get(sessionId)
                  ? cached
                  : { kind: 'no-summary', phase: 'no_task' }
