...
export default {
  ...
  apply(ctx, config) {
    ...
    // When enforceReadonlyPlanning is enabled and the active task is in the
    // planning phase, prune generic write/edit tools from the assembled tool
    // surface. The model keeps read/analysis tools plus trellis_artifact_update
    // (the only sanctioned write channel, physically confined to task docs).
    ctx.on('system-prompt/assemble', async (assembly, context, next) => {
      ...
      if (st.phase === 'planning' || st.phase === 'planning-inline') {
        const PLANNING_SAFE_TOOLS = new Set([
          ...
          'pwsh',
          'bash',
          // workflow diagnostics & advancement
          'trellis_state',
          'trellis_task_update',
          // the ONLY sanctioned write channel (task-dir artifacts only)
          'trellis_artifact_update',
        ])
        if (Array.isArray(assembled.tools)) {
          return {
            ...assembled,
            tools: assembled.tools.filter((t) => t && PLANNING_SAFE_TOOLS.has(t.name)),
          }
        }
      }
      return assembled
